---
title: "Azimuth_Principal_Genes_Filtering"
author: "Enos Kakwambi"
date: "2025-11-05"
output:
  html_document:
    toc: yes
...
input_file<-paste0(here(), "/Azimuth_filtereddatQC_basicQC_LibraryIDs.csv")
truth_file<-paste0(here(), "/Azimuth_ClusterLabels.csv")
name<-"Azimuth"
...
full_true_labels$cells<-gsub("\\.", "-", full_true_labels$cells)
full_true_labels$numeric_labels<-as.numeric(factor(
  full_true_labels$labels,
  levels = unique(full_true_labels$labels)
))
true_labels<-subset(full_true_labels, cells %in% colnames(filtered_counts))
PC1_and_PC2$labels<-true_labels$labels
PC1_and_PC2$numeric_labels <- as.numeric(factor(
  PC1_and_PC2$label,
  levels = unique(true_labels$labels)
))
...
ggplot(PC1_and_PC2, aes(x=PC1, y=PC2, color=labels))+
  ...
  scale_color_brewer(palette="Spectral",
                     name = "Label") +
  theme_minimal()+
  labs(x = "PC1", y = "PC2", title="Azimuth PCA Plot")+
  theme(plot.title=element_text(hjust=0.5))

ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PCA_Plot.jpg"),, 
     dpi = 300)
```

##SK_MEANS CLUSTERING USING CUMULATIVE SCORE
```{r sk-means-clustering} 
source(paste0(here(), "/clSensitivity.R"))
library(skmeans)

# Define the thresholds (scaled values corresponding to 5%, 10%, ... 100%)
thresholds <- seq(10, 50, by = 1)
...
for(i in 1:length(selectedGeneList)){
  set.seed(12345)
  PG_groups_sk[[i]]<-skmeans(x = t(log2(count_matrix[names(selectedGeneList[[i]]),] + 1)) , k = k_cls, control = list(verbose = FALSE))$cluster
  print(i)
}
#PG_groups_sk[[20]]<-skmeans(x=t(log2(count_matrix + 1)), k=k_cls, control = list(verbose = FALSE))$cluster
...
celltype_vec <- setNames(full_true_labels$labels, full_true_labels$cells)
...
seuratObject<- FindVariableFeatures(norm_seuratObject, selection.method = "dispersion", nfeatures = length(row.names(count_matrix))) #dispersion values is log(variance/mean)
...
for(i in seq_along(num_genes)){
  set.seed(12345)
  HVG_groups_sk[[i]]<-skmeans(x = t(log2(count_matrix[HVGs[1:as.numeric(num_genes[i])], ] + 1)) , k = k_cls, control = list(nruns=1, verbose = FALSE))$cluster
  print(num_genes[i])
}
#HVG_groups_sk[[20]]<-skmeans(x=t(log2(count_matrix + 1)), k=k_cls, control = list(verbose = FALSE))$cluster
...
for(i in 1:length(selectedGeneList)){
  set.seed(12345)
  Union_groups_sk[[i]]<-skmeans(x = t(log2(count_matrix[pg_hvg_union[[i]],] + 1)) , k = k_cls, control = list(verbose = FALSE))$cluster
  print(i)
}
...
ggplot(plot_df1, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       ...
       title = "SK-means Clustering Sensitivity for Azimuth Dataset: Loess Regression") +
  scale_color_manual(
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Short_Azimuth_PG_HVG_UNION_Sensitivity.jpg"),, 
     dpi = 300)
...
ggplot(plot_df, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "SK-means Rand Index for Azimuth Dataset: Loess Regression") +
  scale_color_manual(
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Short_Azimuth_PG_HVG_UNION_RandIndex.jpg"),, 
     dpi = 300)
...
ggplot(plot_df1_filtered, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Clustering Sensitivity",
       color = "Gene Selection Method",
       title = "SK-means Clustering Sensitivity for Azimuth Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_sensitivity","HVG_sensitivity","PG_HVG_Union"),
    values = c(
      "PG_sensitivity"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PG_HVG_UNION_No_Outliers_Sensitivity.jpg"),, 
     dpi = 300)
...
ggplot(plot_df_filtered, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "SK-means Clustering Rand Index for Azimuth Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_randIndex","HVG_randIndex","PG_HVG_UnionRand"),
    values = c(
      "PG_randIndex"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PG_HVG_UNION_No_Outliers_RandIndex.jpg"),, 
     dpi = 300)
...
write.csv(intersect_df, file = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PPS_HVG_Variance_Mean_Intersect.csv"))
write.csv(raw_scores_df, file = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PPS_HVG_Variance_Mean_Top100.csv"))
```
...
ggplot() +
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PS_Dispersion_Comparison.jpg"),, 
     dpi = 300)
...
ggplot() +
  ...
  labs(
    ...
    y = "log10 variance",
    title = "Azimuth Top 20 genes: Log10 mean expression vs variance"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c(
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PG_HVG_INTERSECT_Log10_mean_variance.jpg"),, 
     dpi = 300)
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PG_Only_Violin_Plot.jpg"),, 
     dpi = 300)

VlnPlot(norm_seuratObject, features=hvg_only_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_HVG_Only_Violin_Plot.jpg"),, 
     dpi = 300)

VlnPlot(norm_seuratObject, features=common_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/Azimuth_Results/Azimuth_PG_HVG_INTERSECT_Violin_Plot.jpg"),, 
     dpi = 300)
```
