---
title: "Azimuth_MMC_Bootstrapping"
output:
  html_document:
    toc: yes
    toc_float: yes
...
#Install R packages
```{r install-packages}
pkgs <- c("here", "ggplot2", "dplyr", "Seurat", "skmeans", "mclust", "tidyr", "ggrepel", "stringr")
...
principalgenes<- function(data = NULL, pcs = -1, cut = -1, filters=c("IgFilter", "TrFilter", "riboFilter", "mtFilter", "gmFilter", "rikFilter", "dotFilter")) {
  ...
  if(pcs == -1 || pcs >= 20 || pcs >= 0.05*ncol(data)){
    temp<- irlba::prcomp_irlba(t(log2(data+1)), n = min(20, 0.05*ncol(data)), retx = TRUE, center = TRUE, scale. = TRUE)
    eig = temp$sdev^2
    pcs = quick.elbow(eig)
    rm(eig, temp)
  }
  ...
  rotated_output <- irlba::prcomp_irlba(t(log2(data+1)), n = pcs, retx = TRUE, center = TRUE, scale. = TRUE)
...
input_file<-paste0(here(), "/Filtered_Input/Azimuth_filtereddatQC_basicQC_LibraryIDs.csv")
truth_file<-paste0(here(), "/Filtered_Input/Azimuth_ClusterLabels.csv")
name<-"Azimuth"

count_matrix<-read.csv(input_file, check.names = FALSE, row.names = 1)
```

##Get principal genes and highlight them on the log_var_mean_plot
```{r principal-genes-running}
...
#get true labels
full_true_labels<-read.csv(truth_file, check.names = FALSE)
...
ggsave(filename = paste0(here(), "/Bootstrapping_Results/Azimuth_Results/Azimuth_PCA_Plot.jpg"),, 
     dpi = 300)
```
...
names(scored_hvg) <- rownames(count_matrix)
...
write.csv(plot_df1, file = paste0(here(), "/Bootstrapping_Data_Hold/Azimuth_PG_HVG_Union_CumulativeScore_Subsampled_Accuracy.csv"))
```
...
ggsave(filename = paste0(here(), "/Bootstrapping_Results/Azimuth_Results/Azimuth_PG_HVG_Union_CumulativeScore_Subsampled_Sensitivity.jpg"),, 
     dpi = 300)
```