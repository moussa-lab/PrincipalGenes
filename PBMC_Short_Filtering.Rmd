...
input_file<-paste0(here(), "/PBMCSC1_smaller_count_matrix.csv")
truth_file<-paste0(here(), "/PBMCsFromSC1_ClusterLabels.csv")
name<-"PBMCSC1"
...
##PCA PLOT
```{r pca-plot}
PC1_and_PC2<-as.data.frame(principal_genes_results$rotated_output$x, row.names = colnames(count_matrix))
corrects<- c("Regulatory", "Memory", "Naive_Cytotoxic", "B_Cells", "CD14_Monocytes", "NaturalKiller", "Cytotoxic")

#get true labels
true_labels<-read.csv(truth_file)
true_labels$X<-gsub("\\.", "-", true_labels$X)
PC1_and_PC2$labels<-true_labels$x
PC1_and_PC2$label<-factor(PC1_and_PC2$label, levels=1:7, labels=corrects)

#PCA PLOT
ggplot(PC1_and_PC2, aes(x=PC1, y=PC2, color=label))+
  geom_point(size=3)+
  scale_color_brewer(palette="Set1",
                     name = "Label") +
  theme_minimal()+
  labs(x = "PC1", y = "PC2", title="PBMC PCA Plot")+
  theme(plot.title=element_text(hjust=0.5))
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_PCA_Plot.jpg"),, 
    dpi = 300)
```
...
#define the thresholds (scaled values corresponding to 10%, 12%, ... 50%)
...
#store the number of genes at each cumulative score level. used for plots and replication with other gene selection methods
num_genes <- list()
...
#sanity check for cumulative score
sumprop<- 0
for (i in seq_along(principal_genes_results$top_genes)){
  sumprop <- sumprop + principal_genes_results$top_genes[i]*100
}

true_labels_vector <- setNames(true_labels$x,true_labels$X)
k_cls <- max(true_labels_vector)

PG_groups_sk <- list() #sk means clusters of cells based on selected top genes
PG_groups_lv<-list() # louvain clusters of cells based on selected top genes
PG_results_sk <-list() #store clustering sensitivity results for sk means
PG_results_lv <-list() #store clustering sensitivity results for louvain

# SKmeans and louvain clustering
set.seed(12345)
...
for (i in 1:length(selectedGeneList)){
  ...
  PG_results_lv[[i]] <- clustSensitivity(PG_groups_lv[[i]], true_labels_vector, as.character(1:k_cls))
}

#save dataframe comparing labels from truth clusters to labels from sk means clustering using genes that add up to 10% cumulative score.
...
celltype_vec <- setNames(true_labels$x, true_labels$X)
...
seuratObject<- CreateSeuratObject(counts = as.matrix(count_matrix)) # create seurat object
norm_seuratObject <- NormalizeData(seuratObject) #normalize counts
seuratObject<- FindVariableFeatures(norm_seuratObject, selection.method = "dispersion", nfeatures = length(principal_genes_results$top_genes)) #dispersion values is log(variance/mean), select top HVGs
HVGs <- VariableFeatures(seuratObject) #Vector of descending order HVGs
HVGs<-gsub("-", "_", HVGs) #formatting of gene names

positions<-match(HVGs, rownames(count_matrix)) #check if all selected HVGs show up in count_matrix (double check renaming)

scored_hvg<- seuratObject[["RNA"]]@meta.data$vf_disp_data_mvp.dispersion[positions] #extract dispersion scores
names(scored_hvg) <- HVGs
scored_hvg[is.na(scored_hvg)] <- 0 #since dispersion is a fraction, convert unscored genes to 0 value for sorting
scored_hvg <- sort(decreasing = TRUE, scored_hvg)
commongenes<-intersect(names(principal_genes_results$top_genes), names(scored_hvg))#genes selected by both hvg and principal genes

#COMPARE HVGs TO PGs list
setdiff(HVGs, names(unlist(selectedGeneList)))
...
for(i in seq_along(num_genes)){
  set.seed(12345)
  submat<-count_matrix[HVGs[1:as.numeric(num_genes[i])], ]
  HVG_groups_sk[[i]]<-skmeans(x = t(log2(submat + 1)) , k = k_cls, control = list(nruns=1, verbose = FALSE))$cluster
  d<-as.dist(skmeans_xdist(t(log2(submat + 1))))
  d <- 1 - as.matrix(d)
  g <- graph_from_adjacency_matrix(d, mode=c("undirected"), weighted=TRUE, diag=FALSE)
  louvain_cl <- cluster_louvain(g)
...
##SK MEANS CLUSTERING USING UNION OF HVGS AND PBMCS AT EACH POINT
```{r sk-means-union}
pg_hvg_union<-list()

#at the number of genes corresponding to each cumulative score threshold, add on the genes selected by HVGs 
for(i in seq_along(selectedGeneList)){
...
RowNames <- list()
pg_points_sk <- list() #will contain overall clustering sensitivity (true positive labels over all) from sk means clustering results with different subsets of the chosen principal genes
hvg_points_sk <- list() # '' HVGs
union_points_sk<-list() # '' genes by both hvgs and principal genes at each step (defined by number of genes selected by principal genes based on cumulative score)
...
write.csv(points_sk, file=paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_SKM_accuracy_results.csv"))
...
ggplot(plot_df1, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       ...
       title = "SK-means Clustering Sensitivity for PBMC Dataset: Loess Regression") +
  scale_color_manual(
  ...
  theme_minimal()
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_PG_HVG_UNION_Sensitivity.jpg"),, 
   dpi = 300)
...
write.csv(points_sk, file=paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_SKM_rand_index_results.csv"))
...
ggplot(plot_df, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "SK-means Rand Index for PBMC Dataset: Loess Regression") +
  scale_color_manual(
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_PG_HVG_UNION_RandIndex.jpg"),, 
dpi = 300)

```
##NO OUTLIERS
```{r plot-sk-means-no-outliers}

#plot same graph as before but with outliers outside the .95 CI removed
...
ggplot(plot_df1_filtered, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Clustering Sensitivity",
       color = "Gene Selection Method",
       title = "SK-means Clustering Sensitivity for PBMC Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_sensitivity","HVG_sensitivity","PG_HVG_Union"),
    values = c(
      "PG_sensitivity"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_PG_HVG_UNION_No_Outliers_Sensitivity.jpg"),, 
...
ggplot(plot_df_filtered, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "SK-means Clustering Rand Index for PBMC Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_randIndex","HVG_randIndex","PG_HVG_UnionRand"),
    values = c(
      "PG_randIndex"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_PG_HVG_UNION_No_Outliers_RandIndex.jpg"),, 
    dpi = 300)
...
write.csv(points_lv, file=paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_Louvain_accuracy_results.csv"))
...
ggplot(plot_df1, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Clustering Sensitivity",
       color = "Gene Selection Method",
       title = "Louvain Clustering Sensitivity for PBMC Dataset: Loess Regression") +
  scale_color_manual(
    breaks = c("PG_sensitivity","HVG_sensitivity","PG_HVG_Union"),
    values = c(
      "PG_sensitivity"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_Louvain_PBMC_PG_HVG_UNION_Sensitivity.jpg"),, 
...
write.csv(points_sk, file=paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_PBMC_Louvain_rand_index_results.csv"))
...
ggplot(plot_df, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "Louvain Rand Index for PBMC Dataset: Loess Regression") +
  scale_color_manual(
    breaks = c("PG_randIndex","HVG_randIndex","PG_HVG_UnionRand"),
    values = c(
      "PG_randIndex"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_Louvain_PBMC_PG_HVG_UNION_RandIndex.jpg"),, 
       dpi = 300)

```
##NO OUTLIERS
```{r plot-louvain-outliers}
add_loess_ci <- function(df, span = 1, degree = 1, surface = "direct") {
  mod <- loess(sensitivity ~ thresholds, data = df,
               span = span, degree = degree, surface = surface)
  pr  <- predict(mod, newdata = data.frame(thresholds = df$thresholds), se = TRUE)
...
ggplot(plot_df1_filtered, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Clustering Sensitivity",
       color = "Gene Selection Method",
       title = "Louvain Clustering Sensitivity for PBMC Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_sensitivity","HVG_sensitivity","PG_HVG_Union"),
    values = c(
      "PG_sensitivity"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_Louvain_PBMC_PG_HVG_UNION_No_Outliers_Sensitivity.jpg"),, 
        dpi = 300)
...
ggplot(plot_df_filtered, aes(x = thresholds, y = adjustedRandIndex, color = metric)) +
  ...
  labs(x = "Cumulative Score",
       y = "Rand Index",
       color = "Gene Selection Method",
       title = "Louvain Clustering Rand Index for PBMC Dataset: Loess Regression Outliers Removed") +
  scale_color_manual(
    breaks = c("PG_randIndex","HVG_randIndex","PG_HVG_UnionRand"),
    values = c(
      "PG_randIndex"  = "#1BB7D8",
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_Louvain_PBMC_PG_HVG_UNION_No_Outliers_RandIndex.jpg"),, 
        dpi = 300)

```

##PRINCIPAL GENES AND HVGS COMPARISON
```{r compare-gene-names}
...
write.csv(intersect_df, file = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_pbmc_PPS_HVG_Variance_Mean_Intersect.csv"))
write.csv(raw_scores_df, file = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/Short_pbmc_PPS_HVG_Variance_Mean_Top100.csv"))
```
...
ggplot() +
  ...
  labs(
    x = "Principal Score",
    y = "Dispersion",
    title = "PBMC: Principal Score vs Dispersion — Top 20 highlighted by method"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c(
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_PS_Dispersion_Comparison.jpg"),, 
...
ggplot() +
  ...
  labs(
    ...
    y = "log10 variance",
    title = "PBMC Top 20 genes: Log10 mean expression vs variance"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c(
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_PG_HVG_INTERSECT_Log10_mean_variance.jpg"),, 
     dpi = 300)
...
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_PG_Only_Violin_Plot.jpg"),, 
     dpi = 300)

VlnPlot(norm_seuratObject, features=hvg_only_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_HVG_Only_Violin_Plot.jpg"),, 
     dpi = 300)

VlnPlot(norm_seuratObject, features=common_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())
ggsave(filename = paste0(here(), "/Post_Log2_Norm/Filtered/PBMC_RESULTS/PBMC_PG_HVG_INTERSECT_Violin_Plot.jpg"),, 
     dpi = 300)
```
