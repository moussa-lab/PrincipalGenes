...
```{r main-algo}
principalgenes<- function(count_matrix = NULL, pcs = -1, cut = -1, filters=c("IgFilter", "TrFilter", "riboFilter", "mtFilter", "gmFilter", "rikFilter", "dotFilter")) {
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PCA_Plot.jpg"),, 
    dpi = 300)
```
...
celltype_vec <- setNames(true_labels$label, true_labels$cell)
labels_vs_clust$celltype <- celltype_vec[rownames(labels_vs_clust)]
write.csv(labels_vs_clust, paste0(here(), "/Main_Data_Hold/", name, "_truth_vs_clustering.csv"))
smaller_count_matrix<-count_matrix[names(principal_genes_results$top_genes[1:100]),]
write.csv(smaller_count_matrix, paste0(here(), "/Main_Data_Hold/", name, "_top100PG_count_matrix.csv"))
```
...
true_labels_vector <- setNames(full_true_labels$numeric_labels, full_true_labels$cell)
for (i in seq_along(num_genes)){
...
write.csv(points_sk, file=paste0(here(), "/Main_Data_Hold/PBMC_SKM_accuracy_results.csv"))
...
ggplot(plot_df1, aes(x = thresholds, y = sensitivity, color = metric)) +
  ...
  theme_minimal() + theme(legend.position = "none")
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_UNION_Sensitivity.jpg"),, 
   dpi = 300)
...
write.csv(points_sk, file=paste0(here(), "/Main_Data_Hold/PBMC_SKM_rand_index_results.csv"))
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_UNION_RandIndex.jpg"),, 
dpi = 300)
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_UNION_No_Outliers_Sensitivity.jpg"),, 
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_UNION_No_Outliers_RandIndex.jpg"),, 
    dpi = 300)
...
write.csv(intersect_df, file = paste0(here(), "/Main_Data_Hold/PBMC_PPS_HVG_Variance_Mean_Intersect.csv"))
write.csv(raw_scores_df, file = paste0(here(), "/Main_Data_Hold/PBMC_PPS_HVG_Variance_Mean_Top100.csv"))
```
...
ggplot() +
  ...
  theme(plot.title = element_text(hjust = 0.5))
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PS_Dispersion_Comparison.jpg"),, 
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_INTERSECT_Log10_mean_variance.jpg"),, 
     dpi = 300)
...
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_Only_Violin_Plot.jpg"),, 
     dpi = 300)
...
VlnPlot(norm_seuratObject, features=hvg_only_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        ...
        axis.title.y = element_blank(),
        legend.position = "none")
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_HVG_Only_Violin_Plot.jpg"),, 
     dpi = 300)
...
VlnPlot(norm_seuratObject, features=common_gene_names[1:9]) & 
  theme(axis.title.x = element_blank(),
        ...
        axis.title.y = element_blank(),
        legend.position = "none")
ggsave(filename = paste0(here(), "/Sensitivity_ARI_Results/PBMC_Results/PBMC_PG_HVG_INTERSECT_Violin_Plot.jpg"),, 
     dpi = 300)
```
